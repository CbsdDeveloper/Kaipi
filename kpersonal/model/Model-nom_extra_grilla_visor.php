...
$bd->conectar($_SESSION['us'],$_SESSION['db'],$_SESSION['ac']);
        ...
        $_SESSION['id_config'] = $_GET["id_config"];
        
        
        $_SESSION['id_rol'] = $_GET["id_rol"];
        
        $id_config = $_SESSION['id_config'];
 
   //     K_procesa($bd,$obj, $_GET["id_rol"], $_GET["id_config"], $_GET["id_departamento"] );

        if (!empty($where)){
        
                $sql = 'SELECT a.id_rold as "Id",b.fecha as "Fecha Ingreso", a.idprov as "Identificacion",
            			 		 		b.razon as "Nombre",b.unidad as "Departamento", b.cargo as "Cargo",  a.ingreso as "Monto"
    ...
    while ($x=$bd->obtener_fila($stmt)){
        ...
        if ($rol_valida["numero"] == 0){
            ...
            if ($estructura_sistema == 'Sistema'){
                if ( $tipoformula == 'RS'){
                    $ingreso =  sueldo_mes( $bd ,$rol["id_periodo"], $id_rol,trim($x['idprov']) ,$rol["anio"],$rol["mes"],$x['sueldo']);
                   
                }
                
                if ( $tipoformula == 'HS'){
                    $ingreso = horas_suplementarias( $bd ,$rol["id_periodo"], $id_rol,trim($x['idprov']) ,$rol["anio"],$rol["mes"]);
                }
                
                if ( $tipoformula == 'HE'){
                    $ingreso = horas_extras( $bd ,$rol["id_periodo"], $id_rol,trim($x['idprov']) ,$rol["anio"],$rol["mes"]);
                }
                
                if ( $tipoformula == 'FR'){
                    $ingreso = FondosReserva( $bd ,$rol["id_periodo"], $id_rol,trim($x['idprov']) ,$rol["anio"],$rol["mes"]);
                }
...
///--------
function horas_extras( $bd ,$id_periodo, $id_rol,$idprov ,$anio,$mes ){
...
//-------------------
function sueldo_mes( $bd ,$id_periodo, $id_rol,$idprov ,$anio,$mes,$sueldo ){
    ...
    if (!empty($User['dias'])){
        ...
        if ( $User['dias'] == 30 ){
            ...
            $valor_parcial = ( $User['dias'] * $User['sueldo'] ) / 30; 
