...
function goToURL(accion,id) {
	 ...
	 var parametros = {
	  ...
	  $.ajax({
					data:  parametros,
					url:   '../model/Model-nom_accion.php',
					type:  'GET' ,
					cache: false,
					beforeSend: function () { 
...
function openFile(url) {
	  ...
	  var id = $('#id_accion').val();

	  var motivo = $('#motivo').val();
	  

	  if ( motivo == 'JUBILACIÓN') {
	    	enlace = '../../reportes/accion_v1'+'?id='+id  ;
	   }else
	   {
			enlace = url+'?id='+id  ;
	   }
...
function BusquedaGrilla(oTable){        	 
		...
		$.ajax({
		 	data:  parametros,
		    url: '../grilla/grilla_nom_accion.php',
			dataType: 'json',
			cache: false,
			success: function(s){
			//console.log(s); 
...
function FormView()
{
	 $("#ViewForm").load('../controller/Controller-nom_accion.php');
}
//----------------------
function FormFiltro()
{
	 $("#ViewFiltro").load('../controller/Controller-nom_accion_filtro.php');
}
