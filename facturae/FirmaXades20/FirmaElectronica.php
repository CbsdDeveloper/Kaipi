...
{           
    ...
    //private $signer="https://ensystems-sri-xades.herokuapp.com/WS/firmaXadesBes.wsdl"; // WS de firmado, EN Systems
    private $signer="https://exa-xadesbes.herokuapp.com/FirmaXades/firmaXadesBes.wsdl"; // WS alterno
    //private $signer="http://contamax.herokuapp.com/WS/firmaXadesBes.wsdl"; // WS alterno
    private $validator="https://ensystems-sri-xades.herokuapp.com/WS/validaXadesBes.wsdl"; // WS de validation, EN Systems
    private $rideWS="https://aguilerasoftware.herokuapp.com/WS/rideSri.wsdl"; // WS de Creacion de Rides, EN Systems
    // 0=Online, 1=Offline
    ...
    public function __construct(array $config = array()){
        ...
        $this->setAmbiente(false,false,null);
    }
    public function setConfig($config=array()){
         $this->data=array_merge($this->data,$config);
    }
    public function setAmbiente($online=false,$production=false,$signer=null){
        $k=$online==true?0:1;
        ...
        try {
            ...
            if($result['success']==true)
                if(!empty($this->data['file_signed'])&&!empty($result['xml'])){
                    ...
                    $fp = fopen($file, 'wb');
                    fwrite($fp,self::encode_utf8($result['xml'])); fclose($fp);
    ...
    public function autorizarSri($claveAcceso=null, $file_autorized=null, $echo=false){
        try {
            ...
            if($error) return array('success'=>false,'message'=>$error,'type'=>'response');
            // todo correcto
            if($result['RespuestaAutorizacionComprobante']['numeroComprobantes']*1==0) throw new Exception("No se han Encontrado Documentos Para la Clave Consultada!");
