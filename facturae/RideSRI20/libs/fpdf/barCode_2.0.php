...
protected $T128;                                             // tableau des codes 128
    ...
    protected $SetFrom=array();                                          // Convertisseur source des jeux vers le tableau
    protected $SetTo= array();                                            // Convertisseur destination des jeux vers le tableau
    ...
    protected $tablewidths;   // VARIABLE PARA LAS TABLAS
    protected $footerset;     // VARIABLE PARA LAS TABLAS
    ...
    function Row($data,$border=false,$multi=true){
        ...
        if($multi==true){
            ...
            for($i=0;$i<count($data);$i++){
                ...
                if(!empty($f)) $this->SetFont($f[0],$f[1],$f[2]); else $this->SetFont($f1,$f2,$f3);
        ...
        for($i=0;$i<count($data);$i++){
            ...
            if(!empty($f)) $this->SetFont($f[0],$f[1],$f[2]); else $this->SetFont($f1,$f2,$f3); 
            if($multi==true)
                $this->MultiCell($w,$this->C_height,$this->formatText($data[$i]),0,$a);                
            else
    ...
    function formatText($txt){
        $text=!!mb_detect_encoding($txt,'UTF-8',true)?mb_convert_encoding($txt,'ISO-8859-1','UTF-8'):$txt;
        return trim($text);
    }
    ...
    // imagen
    function centerImageFile($file, $x=null, $y=null, $max_w=0, $h=0, $type='', $link=''){// Put an image on the page
        $w=0;
        if(!isset($this->images[$file])){// First use of this image, get info
            if($type==''){
                $pos = strrpos($file,'.');
                if(!$pos)
