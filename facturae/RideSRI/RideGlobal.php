...
class RideGlobal extends PDF_Code128{
	...
	public function createPdf($array, $logo_path=null, $option='I'){
		...
		$this->setField(121,41,"N�MERO DE AUTORIZACI�N:",'B');
		...
		if(!empty($array['fechaAutorizacion'])){
			$this->setField(121,54,"FECHA Y HORA DE AUTORIZACI�N:",'B');
		...
		$this->setField(141,67,$infTrib['ambiente']*1==1?'PRUEBAS':'PRODUCCI�N');
		$this->setField(121,72,"EMISI�N:",'B');
		...
		/* DATOS INFO EMPRESA */
		$this->setField(13,67,$infTrib['razonSocial'],'B',10,null,100);
		$this->setField(13,72,$this->issetText($infTrib,'nombreComercial'),'',9,null,100);
		$this->setField(13,78,"DIRECCI�N:",'B',8);
		...
		switch($array['documento']){
            ...
            case '03': // LIQUIDACION EN COMPRAS
				if(in_array($array['version'],array( '1.0.0', '1.1.0' ))){
					$this->createLiquidacion($datoXml); break;
				}
	...
	*/
	 private function createRetencion2($datoXml){
        $documento="COMPROBANTE DE RETENCI�N";
        ...
        $this->setField(13,103,"RAZ�N SOCIAL:",'B',8);
        ...
        $this->setField(75,108,"FECHA DE EMISI�N:",'B',8);
        ...
        $this->Row(array('Comprobante','N�mero','Fecha Emisi�n','Ejercicio Fiscal','Base Imponible','Impuesto','Porcentaje %','Valor Retenido'),true,false);
        ...
        foreach ($this->formatArray($datoXml['docsSustento']['docSustento'])AS $doc){
            foreach ($this->formatArray($doc['retenciones']['retencion'])AS $impu){
                ...
                $this->Row(array(
                    ...
                    ($impu['codigo']=='1'?'RENTA':'IVA').'  '.($impu['codigo']=='1'?$impu['codigoRetencion']:$this->ret_iva[$impu['codigoRetencion']]),
    ...
    private function createLiquidacion($datoXml){
        $documento="LIQUIDACI�N DE COMPRA DE BIENES Y PRESTACI�N DE SERVICIOS";
		/* Inicio Datos Dedicados */
        ...
        if(strlen($this->issetText($infoFact,"direccionProveedor"))>0){
            $this->setField(13,113,"DIRECCI�N:","B",8);
	...
	private function createFactura($datoXml){
		...
		$this->setField(75,108,"FECHA DE EMISI�N:",'B',8);
		$this->setField(108,108,$infoFact['fechaEmision']);
		$this->setField(142,108,"GU�A DE REMISI�N:",'B',8);
    ...
    private function createDetalleFactLiqui($datoXml,$infoFact){
		...
		$this->SetCAligns('C');
		$this->SetFont('Arial','B',8);
		$this->SetCWidths(array(8,13,18,100,20,15,20));            
		$this->Row(array('No.','C�DIGO','CANTIDAD','DESCRIPCI�N','PRECIO U.','DESC.','TOTAL'),true,false);
		$this->SetFont('Arial','',8);
		$this->SetCAligns(array('L','C','C','L','R','R','R'));
		$this->SetCHeight(5);
		foreach ($this->formatArray($datoXml['detalles']['detalle'])AS $i=>$deta){
			$this->Row(array(($i+1),$this->issetText($deta,'codigoPrincipal'),$deta['cantidad'],$deta['descripcion'].$this->detallesAdi($deta),$deta['precioUnitario'],$deta['descuento'],number_format($deta['precioTotalSinImpuesto'],2) ) ,true,true);
		}
		...
		if(isset($datoXml['reembolsos'])){
		   ...
		   $reembolsos=$this->formatArray($datoXml['reembolsos']['reembolsoDetalle']);
	...
	private function createNotaCredito($datoXml){
		$documento="N O T A       DE       C R � D I T O";
		/* Inicio Datos Dedicados */
		...
		$this->setField(13,120,"FECHA DE EMISI�N (Comprobante a modificar):",'B');
		$this->setField(95,120,$infoNCred['fechaEmisionDocSustento']);
		$this->setField(13,125,"RAZON DE MODIFCACI�N:",'B');
		...
		$this->Row(array('No.','C�DIGO','DESCRIPCI�N','CANTIDAD','PRECIO U.','TOTAL'),true,false);
		$this->SetFont('Arial','',8);
	...
	private function createNotaDebito($datoXml){
		$documento="N O T A        DE        D � B I T O";
		/* Inicio Datos Dedicados */
		...
		$this->Row(array('No.','RAZON DE LA MODIFICACION','VALOR DE LA MODIFICACI�N'),true,false);
		$this->SetFont('Arial','',8);
	...
	private function createGuiaRemi($datoXml){
		...
		$this->setField(13,103,"IDENTIFICACI�N (TRANSPORT�STA):",'B',8);
		$this->setField(75,103,$infoGuia['rucTransportista'],'',8);
		$this->setField(13,108,"RAZ�N SOCIAL / NOMBRES Y APELLIDOS:",'B',8);
		...
		foreach($this->formatArray($datoXml['destinatarios']['destinatario']) AS $desti){
			...
			$this->setField(132,$iapos+2,"FECHA DE EMISI�N:",'B',8);
			$this->setField(13,$iapos+7,"N�MERO DE AUTORIZACI�N:",'B',8);
			...
			$this->setField(13,$iapos+22,"IDENTIFICACI�N (DESTINATARIO):",'B',8);
			$this->setField(75,$iapos+22,strtoupper($desti['identificacionDestinatario']),'',8);
			
			$this->setField(13,$iapos+27,"RAZ�N SOCIAL / NOMBRES Y APELLIDOS:",'B',8);
			...
			$this->setField(13,$iapos+37,"C�DIGO ESTABLECIMIENTO DESTINO:",'B',8);
			...
			$this->Row(array('No','C�D. PRINCIPAL','CANTIDAD','DESCRIPCI�N'),true,false);
	...
	private function infoAddi($pos,$dato){
		if($pos<12) $pos=12;
		$this->setY($pos+2);    
		$this->setField(14,null,"INFORMACI�N ADICIONAL",'B',9,3);    
		$this->RoundedRect(10, $this->getY()-7 , 125, 6, 0, '');   
		
		$this->SetCWidths(array(25,100));
		$this->SetCAligns(array());
		$this->SetCFonts(array(array('Arial','B',7),array('Arial','',7)));    
		$this->RectCof(10, $this->GetY()-1, 125, 1, 'LR');
		foreach($this->formatArray($dato) AS $v){        
			$h=$this->Row(array(strtoupper($v['@attributes']['nombre']).(trim($v['@attributes']['nombre'])==''?'':':'),$v['@value'].''));
