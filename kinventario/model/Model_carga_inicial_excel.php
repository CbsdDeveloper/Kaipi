 ...
 if(isset($_POST['submit']))
{
 ...
 //   $chk_ext = explode(".",$fname);
    ...
    echo 'Referencia Id. '.$idbodega.'-'.$ruc_registro;
        ...
        if (!empty($filename)){
                    ...
                    while (($data = fgetcsv($handle, 1000, ";")) !== FALSE)
                    {
                        ...
                        $codigoi      =  (trim($data[11]));
                        $marca      =  strtoupper(utf8_encode(trim($data[12])));
                        $barra      =  strtoupper(utf8_encode(trim($data[13])));
                     
                        $nombre_producto = $producto  ;
                        
                        
                        $id_producto = _busca_producto($bd,$nombre_producto,$ruc_registro,$idbodega);
                                             ...
                                             if ( empty($id_producto)) {
                                                 ...
                                                 if ($lon1  > 1) {
                                                    ...
                                                    $id_producto = _guarda_producto($bd,$nombre_producto ,
                                                                                    ...
                                                                                    $unidad, $facturacion,$tributo,$costo,
                                                                                    $stock,$precio_iva,$ruc_registro,
                                                        $idbodega,$codigoi,$marca,$secuencia,$barra);
                                                     // precio
                                                    if ( $id_producto > 0) {
                        ...
                        echo  ' <br> registros: '.$i.' PROD '. $id_producto ;
 ...
 //-----------------------------------
    function _busca_producto($bd	,$id,$ruc_registro,$idbodega){
              ...
              $AResultado = $bd->query_array(
                ...
                'producto='.$bd->sqlvalue_inyeccion(trim($id),true). ' and 
                 idbodega='.$bd->sqlvalue_inyeccion(trim($idbodega),true). ' and 
  ...
  //---------------
    function _busca_marca($bd	,$marca){
        
             
        $AResultado = $bd->query_array(
            'web_marca',
            'idmarca',
            'nombre='.$bd->sqlvalue_inyeccion(trim($marca),true)
            );
        
        $idcategoria =   $AResultado['idmarca'];
        
        $id = $idcategoria;
        
        
        $InsertQuery = array(
            array( campo => 'nombre',   valor => $marca),
            array( campo => 'referencia',   valor => $marca )
        );
        
        
        if (empty($idcategoria) ){
            
            $id = $bd->JqueryInsertSQL('web_marca',$InsertQuery);
...
//---------------------
    function _guarda_producto($bd,$nombre_producto ,
                                ...
                                $ruc_registro,$idbodega,$codigoi,$marca,$secuencia,$barra){
      ...
      /*  
        ...
        $AResultado = $bd->query_array(
            'web_producto',
            'idproducto',
            'producto='.$bd->sqlvalue_inyeccion(trim($nombre_producto),true).' and
             idbodega='.$bd->sqlvalue_inyeccion(trim($idbodega),true).' and
             registro='.$bd->sqlvalue_inyeccion(trim($ruc_registro),true)
            );
        ...
        $InsertQuery = array(
            ...
            array( campo => 'referencia',   valor => $codigoi.' - '.$producto ),
            ...
            array( campo => 'codigo',        valor => $codigo_item),
            ...
            array( campo => 'codigob',   valor => $codigo_barra),
            array( campo => 'controlserie',   valor => 'N'),
            array( campo => 'cuenta_gas',   valor => '-'),
            array( campo => 'registro',   valor => $ruc_registro),
            array( campo => 'fifo',   valor => $secuencia),
