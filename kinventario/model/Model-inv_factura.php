...
class proceso{
	...
	function proceso( ){
		...
		$this->ATabla = array(
		    ...
		    array( campo => 'carga',   tipo => 'NUMBER',   id => '20',  add => 'S',   edit => 'S',   valor => '-',   filtro => 'N',   key => 'N')
		);
	...
	function consultaId($accion,$id ){
		...
		$datos = $this->bd->JqueryArrayVisorTab('view_ventas_fac',$qquery,'tab1' );
		
		 
		echo "<script>
        		$('#idprov1').val("."'".trim($datos['idprov'])."'"." );
        		$('#razon1').val("."'".trim($datos['razon'])."'"." );
        		$('#correo1').val(". "'".trim($datos['correo'])."'"." );";
	...
	function eliminar($id ){
	    ...
	    if (trim($AEstado['estado'])  == 'digitado'){
	        ...
	        $result = '<b> NO SE PUEDE ELIMINAR EL REGISTRO.... </b> ' ;
	    }
	...
	//-------------
	function AprobarComprobante($accion,$id,$tipo,$idprov,$tipofactura){
	 	...
	 	if (!empty($id)){
	 	    ...
	 	    if ( $tipofactura == '0'){
	 	        ...
	 	        $comprobante_DATO = 'I'.$input ;
	 	         ...
	 	         $estado1 = 'aprobado';
        	        ...
        	        $sql = " UPDATE inv_movimiento
        						   SET 	estado=".$this->bd->sqlvalue_inyeccion($estado1, true).",
        	     ...
        	     //   $this->_Verifica_facturas();
        	        ...
        	        echo '<script type="text/javascript">acciona("'.$comprobante_DATO.'","'.$estado1.'" );</script>';
        	        
        	        $result = '<img src="../../kimages/kfav.png" align="absmiddle"/>&nbsp;<b>COMPROBANTE EMITIDO CON EXITO ['.$comprobante_DATO.']</b><br>';
...
function _Verifica_suma_facturas_Total(   ){
    ...
    $sql_det1 = 'select   id_movimiento
                        FROM inv_movimiento
                        where  (coalesce(iva,0) + coalesce(base0,0) + coalesce(base12,0)) <> total ';
...
$gestion   = 	new proceso;
    ...
    if (isset($_GET['accion']))	{
    	...
    	if ($accion == 'aprobacion'){
    	    ...
    	    $gestion->AprobarComprobante($accion,$id,$tipo,$idprov,$tipofactura);
