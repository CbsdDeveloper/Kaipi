...
class proceso{
	...
	function GrillaPago( $id_importacion){
	    ...
	    $sql = "SELECT a.id_asiento as asiento,
                       a.fecha,  
                       a.idprov || ' ' as proveedor, 
                       b.detalle  as cuenta,  a.detalle, a.debe  as monto,
                       a.marca
                    FROM view_diario a
                    join co_plan_ctas b on  a.idmovimiento = ". $this->bd->sqlvalue_inyeccion($id_importacion , true)." and 
                         a.tipo='M' and 
                         a.debe > 0 and 
                         a.base is null and 
                         a.registro = ". $this->bd->sqlvalue_inyeccion(trim($this->ruc) , true)."  and 
                         a.cuenta= b.cuenta and
                    	 a.registro = b.registro and b.tipo_cuenta <> 'I'";
	      ...
	      $this->obj->grid->KP_GRID_COSTO($resultado,$tipo,'asiento',$formulario,'N','','','','');
