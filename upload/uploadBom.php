...
<?php
	...
	$bd->conectar($_SESSION['us'],$_SESSION['db'],$_SESSION['ac']);
 	...
 	if  (isset($_GET['status'])) {
			    ...
			    $filename = guarda_imagen('sas',$carpeta,$archivo,$bd,$id   ); 
...
<script type="text/javascript">
 ...
 function retorna_valor(symbol) {
	 ...
	 let text = document.getElementById(symbol).value;
	 
	 let result = text.trim();
	 
	 
	 var valor = '../../kimages/' +result ;
	  	 
	 
	   window.opener.document.getElementById('url').src= valor;
	     ...
	     <div class="panel panel-primary">
      			  ...
      			  <div class="panel-heading">Carga de archivos</div>
						...
						<div class="panel-body">
							...
							<form action="uploadBom?status=ok&file=<?php echo $_GET['file'] ?>&id=<?php echo $_GET['id'] ?>" method="post" enctype="multipart/form-data" id="forma">
									   ...
									   <div class="col-md-10" style="padding: 2px"> 
										...
										<div class="col-md-10" style="padding: 2px"> 
												<input name="enviar" type="button" class="btn btn-info" value="copiar" onClick="retorna_valor('archivo_file')" />
									   </div>
   ...
   function guarda_imagen($sesion_variable,$carpeta_variable,$archivo ,$bd,$id   ){
			...
			if($copiado==false){ 
				...
				return "error"; 
				   ...
				   $sql = "UPDATE bomberos.bitacora_bom 
				   			  SET grafico    = ".$bd->sqlvalue_inyeccion($archivo, true).' 
						    WHERE id_bita_bom='.$bd->sqlvalue_inyeccion( $id, true);
           
        
           			$bd->ejecutar($sql); // EJECUTA SENTENCIA SQL  RETORNA RESULTADO
