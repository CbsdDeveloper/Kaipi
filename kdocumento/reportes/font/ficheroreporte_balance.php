...
*/
  ...
  function Footer() { //función footer
    ...
    function cabeceraHorizontal($cabecera,$fuente, $row_col,$proveedor,$pago)
    {
	   ...
	   $MENSAJE = 'BALANCE DE COMPROBACION CONTABLE '.$_SESSION['ireporte'];
    ...
    function datosHorizontal($resultado,$fuente,$row_col)
    {
		...
		$this->CellFitSpace(25,5, 'Cta Contable',1, 0 , 'C', $bandera );
		$this->CellFitSpace(75,5, 'Cuenta',1, 0 , 'C', $bandera );
		
		$this->CellFitSpace(20,5, 'Debe',1, 0 , 'C', $bandera );
		$this->CellFitSpace(20,5, 'Haber',1, 0 , 'C', $bandera );

	    $this->CellFitSpace(20,5, 'Deudor',1, 0 , 'C', $bandera );
		$this->CellFitSpace(20,5, 'Acreedor',1, 0 , 'C', $bandera );
...
//Detalle	Beneficiario	Debe	Haber
	   ...
	   $total2 = 0;
		while ($x=$bd->obtener_fila($resultado)){
			...
			$total3 =  $total3 + $debe1;
			$total4 =  $total4 + $haber1;
				 
			$this->CellFitSpace(25,5, utf8_decode(trim($x['0'])),1, 0 , 'L', $bandera );
			$this->CellFitSpace(75,5, utf8_decode(trim($x['1'])),1, 0 , 'L', $bandera );
			
		
			$this->CellFitSpace(20,5, number_format($debe,2),1, 0 , 'R', $bandera );
			$this->CellFitSpace(20,5, number_format($haber,2),1, 0 , 'R', $bandera );
			
			$this->CellFitSpace(20,5, number_format($debe1,2),1, 0 , 'R', $bandera );
			$this->CellFitSpace(20,5, number_format($haber1,2),1, 0 , 'R', $bandera );
		...
		// total
			...
			$this->CellFitSpace(25,5, '',0, 0 , 'L', $bandera );
			
			$this->CellFitSpace(75,5, 'TOTAL',1, 0 , 'R', $bandera );

		 	$this->CellFitSpace(20,5, number_format($total1),1, 0 , 'R', $bandera );
		 	$this->CellFitSpace(20,5, number_format($total2),1, 0 , 'R', $bandera );

			$this->CellFitSpace(20,5, number_format($total3),1, 0 , 'R', $bandera );
		 	$this->CellFitSpace(20,5, number_format($total4),1, 0 , 'R', $bandera );
