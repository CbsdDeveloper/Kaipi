...
class proceso{
    ...
    function GrillaGasto_periodo_uno( $ffecha1,$ffecha2,$fmodulo,$vunidad){
        ...
        $sql = 'SELECT  fecha,asunto,idcaso, documento, caso, envia, 
                         nombre_solicita ,idcasodoc,secuencia,anio,tipodoc,id_departamento
    ...
    function grilla($resultado,$tipo,$presupuesto,$nombre)  {
        ...
        while($row=pg_fetch_assoc($resultado)) {
            ...
            $unidad  =  trim($row['id_departamento']);
             
            $cod_doc_ca = $row['idcasodoc'];

            $datos_seq      = $this->bd->__documento_secuencia($anio,$tipodoc,$unidad,0);
            $secuencia0     = $datos_seq['secuencia'] ;
          ...
          $boton = '<button onclick="formato_doc_visor('.$row['idcaso'].','.$row['idcasodoc'].');" class="btn btn-xs">
            ...
            $evento2 = 'Pone_doc_user('.$row['idcaso'].','.$row['secuencia'].','."'".trim($row['documento'])."',". $secuencia0.','. $cod_doc_ca  .')';
