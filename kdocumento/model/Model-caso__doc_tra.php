...
class proceso{
    ...
    public function BusquedaDoc( $idcaso){
        ...
        while ($fetch_doc=$this->bd->obtener_fila($resultado_doc)){
            ...
            $boton2 = '<button class="btn btn-xs btn-success"
                            data-toggle="modal"
                            data-target="#myModalDocVisor"
                            title="Documento Relacionado"
                            onClick="PoneDoc('. "'" .trim($fetch_doc['archivo']) ."'". ')">
                           <i class="glyphicon glyphicon-file"></i></button>&nbsp;&nbsp;&nbsp;';
            
            echo ' <tr>';
            echo ' <td>'.$fetch_doc['fecha'].'</td>';
            echo ' <td>'.$fetch_doc['detalle'].'</td>';
            echo ' <td>'.$fetch_doc['sesion'].'</td>';
