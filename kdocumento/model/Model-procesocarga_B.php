...
class proceso{
	...
	//----------------------------------------------------------------------------
	    ...
	    if ($nexiste > 0 ){
			...
			while(!feof($file))
			{
			    $line = stream_get_line($file,10240, '[');
	...
	function DibujarProceso( $id   ){
		...
		while ($x=$this->bd->obtener_fila($stmt)){
		    ...
		    $nombreproceso	  = trim($this->nombreproceso($linea));
		    
		    $parrafoTexto    = explode(".-",trim($nombreproceso));
		    
		    $nombreproceso     = str_replace ( ']', '',$nombreproceso );
		    
		  
		    
		    if(is_numeric($parrafoTexto[0])) {
		        
		        $nombreurl	    = trim($this->nombreurl($linea));
		        
		        $secuencia		= $this->ordensecuencia($linea,$secuenciaid);
		        ...
		        $textoWeb       = trim($actual);
		 	           
		        $UrlFlow        =  '<a href="#">'.$textoWeb.'</a>';
		        $UrlVisor       =  '<a href="'."javascript:Visor(".$secuencia.")".'">'.$textoWeb.'</a>';
		        $Ejecuta		=  '<a href="'."javascript:Ejecuta(".$secuencia.")".'">'.$textoWeb.'</a>';
		        
		        $parametro1     = str_replace ( $textoWeb, $UrlFlow,$linea );
		        $ejecucion1     = str_replace ( $textoWeb, $Ejecuta,$linea );
		        $visor          = str_replace ( $textoWeb, $UrlVisor,$linea );
		 
		   
		    } else {
  		        
		        if ( $i==2){
		            if ( $parrafoTexto[0]=='CDATA'){
	...
	function nombreproceso($linea){
	    ...
	    if (!empty($secuencia)){
	        $texto = strip_tags ($parrafotexto['0']);
	        
	        $textofinal = $secuencia.".- ".$texto;
	...
	//--------------------------------------------------------------------------------
	//--- eliminar de registros
	function ordensecuencia($linea,$secuenciaid){
	    ...
	    $parrafoUr2 = $parrafoSecuencia[$num_tags] ;
	    
	   
	    
	    if (empty($parrafoUr2)){
	        $nombreproceso = $secuenciaid * -1;
	    }else{
	        $nombreproceso = $parrafoUr2;
	    }
