...
class proceso{
    ...
    function edicion_variables( $id_proceso, $idcaso  ){
        ...
        $sql = 'SELECT  variable, valor,   orden,idcaso_var
                  FROM flow.wk_proceso_caso_var
                  where idcaso = '.$this->bd->sqlvalue_inyeccion($idcaso ,true).'  order by orden'    ;
    ...
    function Aprobado( $id,$idproceso,$tarea_actual ,$novedad,$sesion_siguiente,$siguiente ,$sesion_reasigna){
        ...
        if ( $bandera == 1 ){
            ...
            if ( $valida_siguiente >= 1) {
                ...
                $this->_agregar_historico( $tarea_actual, $id ,$sesion_siguiente ,$novedad,'N');
            ...
            //---- reasignar proceso a la unidad
            if ( $bandera == 99 ){
                $timestamp = date('H:i:s');
                $fecha     = $this->bd->hoy();
                
                $sql1 = " UPDATE flow.wk_proceso_casotarea
        			        SET sesion_actual=".$this->bd->sqlvalue_inyeccion($sesion_reasigna, true).",
                                hora_in=".$this->bd->sqlvalue_inyeccion($timestamp, true).",
                                novedad=".$this->bd->sqlvalue_inyeccion($novedad, true).",
                ...
                $reasigna = $this->bd->__user($sesion_reasigna);
                
                
                $this->_agregar_historico( $tarea_actual, $id ,$sesion_reasigna ,$novedad,'S');
                
                $dato   = $this->notificacion_email($id,$sesion_reasigna,$reasigna['nombre_siguiente'],'N');
                
                $result = '<img src="../../kimages/kfav.png" align="absmiddle"/>&nbsp;<b>PROCESO CON EXITO ['.$id.']... TRAMITE EN REASIGNADO</b> '.$dato;
