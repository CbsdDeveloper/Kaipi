$(document).ready(function(){
...
});  //-------------------------------------------------------------------------  function modulo()
 {
	 ...
	 $("#Filtrolibro").load('../controller/Controller-co_analisis_libro_nic.php');
...
//----------------------
 ...
 function ResumenFinanciero()
 {
 	...
 	$.ajax({
			data:  parametros,
			 url:   '../model/Model-resumen_nic.php',
			type:  'POST' ,
			cache: false,
			beforeSend: function () { 
...
//-------------------------------------------------------------------------  function Archivo_balance(tipo) {	 	  var ffecha2 = $("#ffecha2").val();	   			if ( ffecha2){ 						  var url = '../model/archivo_finanzas?id='+tipo+'&periodo=' + ffecha2;			  		 	  window.open(url ,'#','width=750,height=480,left=30,top=20');		 	  		}	 				 } //---------------------------------------------------------
 function ResumenResultados()
 {
 	 ...
 	 var parametros = {
			    'ffecha1' : ffecha1,  			    'ffecha2' : ffecha2, 			    'com1' : com1     };
 	$.ajax({			data:  parametros,			url:   '../model/Model-ResultadosEsigef_nic.php',			type:  'POST' ,			cache: false,			beforeSend: function () { 
...
//-------------------------------------------------------------------------   //------------------------------------------------------------------------- function ResumenBalance(){
 	...
 	$.ajax({
			data:  parametros,
			 url:   '../model/Model-Balance_Esigef_nic.php',
			type:  'POST' ,
			cache: false,
			beforeSend: function () { 
...
//------------------------------------------------------------------------- 
 function ResumenBalanceG(){
 	...
 	$.ajax({			data:  parametros,			 url:   '../model/Model-BalanceSituacion_Esigef_nic.php',			type:  'POST' ,			cache: false,			beforeSend: function () { 						$("#ViewBalanceSituacion").html('Procesando');
				},			success:  function (data) {
...
//------------------------------------------------------------------------- 
 function ResumenFlujo(){
 	...
 	$.ajax({
			data:  parametros,
			 url:   '../model/Model-FlujoEsigef_nic.php',
			type:  'POST' ,
			cache: false,
			beforeSend: function () { 
