...
$(document).ready(function(){
       ...
       var oTable = $('#jsontable').dataTable(); 
		...
		$("#FormPie").load('../view/View-pie.php');				$("#ViewFiltroCaja").load('../controller/Controller-co_caja_disponible.php'); 		
		modulo();
...
//--------------
 ...
  function GuardaCaja( ) { 	   var cuentaa  =  $("#cuentaa").val();    	   var partidaa =  $("#partidaa").val();    	   var montoa   =  $("#montoa").val();    	    	     	    var parametros = { 				 'accion' : 'agregar' ,                 'cuentaa' : cuentaa ,                 'partidaa':partidaa,                 'montoa':montoa,	  };	   if ( montoa > 0){	 		      $.ajax({								data:  parametros,						url:   '../model/ajax_CajaContable.php',		 				type:  'GET' ,		 				cache: false,		 				beforeSend: function () { 		 							$("#ViewFormCaja").html('Procesando');		 					},		 				success:  function (data) {		 						 $("#ViewFormCaja").html(data);   		 						 $('#myModal').modal('hide');		 					} 						}); 	   } } //------------ function elimina_caja(accion,id ) {  	    var parametros = { 				 'accion' : accion,                 'id' : id   	  }; 	 		      $.ajax({								data:  parametros,						url:   '../model/ajax_CajaContable.php',		 				type:  'GET' ,		 				cache: false,		 				beforeSend: function () { 		 							$("#ViewFormCaja").html('Procesando');		 					},		 				success:  function (data) {		 						 $("#ViewFormCaja").html(data);    		 					} 						});   } //----------------------------
 function CajaAplica( ) {  	    var parametros = { 				'accion' : 'visor' 	  };	       $.ajax({				data:  parametros, 				url:   '../model/ajax_CajaContable.php', 				type:  'GET' , 				cache: false, 				beforeSend: function () {  							$("#ViewFormCaja").html('Procesando'); 					}, 				success:  function (data) { 						 $("#ViewFormCaja").html(data);    					} 		});   }
