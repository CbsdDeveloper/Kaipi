...
class Model_listaAuxqSRI_marca{
	...
	//-----------------------------------------------------------------------------------------------------------
	function Model_listaAuxqSRI_marca( ){
	...
	function GrillaMaestro( $idprov,$anio,$cuenta,$bandera,$cmes,$marca){
	    ...
	    while ($x=$this->bd->obtener_fila($stmt)){
	        ...
	        $this->GrillaPago( $idprov,$cuenta,$cmes,$anio,$marca);
 	...
 	//---------------------------------
	function GrillaPago( $idprov,$cuenta,$cmes,$anio,$marca){
        ...
        <thead> <tr>';
            ...
            while($row=pg_fetch_assoc($resultado)) {
                ...
                $this->Actualiza_datos(trim($marca), $row['id_asientod'],$idprov);
...
//-----------------
	function Actualiza_datos($estado, $id_asientod,$proveedor){
                ...
                if (trim($estado) == '1'){
                    ...
                    $sql = "update co_asiento_aux 
                            ...
                            where id_asientod=".$this->bd->sqlvalue_inyeccion($id_asientod, true).' and 
                                idprov='.$this->bd->sqlvalue_inyeccion(trim($proveedor), true) ;
...
$gestion   = 	new Model_listaAuxqSRI_marca;
...
if (isset($_GET["prove"]))	{
    ...
    $gestion->GrillaMaestro( $idprov,$anio,$cuenta,$bandera,$cmes,$marca);
