...
class proceso{
    ...
    function grilla( $f1,$f2,$tipo,$nivel){
        ...
        */
        ...
        if (!empty($sql)){
            ...
            $this->cabecera();
            $this->activos_tabla( $sql,$bandera,$nivel);
            ...
            $this->cabecera();
            $this->pasivo_tabla( $sql2,$bandera,$nivel);
    ...
    function sqlwhere($f1,$f2,$tipo,$nivel){
        ...
        $cadena1  = "( tipo_cuenta = 'I') and ";
        $cadena11 = "( tipo_cuenta = 'G' ) and ";
        ...
        $where1 = $cadena11.$cadena2.$cadena3;
        ...
        $wheref = '( fecha BETWEEN '.$this->bd->sqlvalue_inyeccion(trim($f1),true)." and ".
            ...
            if ( $bandera == 0 ){
                ...
                $sql2 = 'SELECT cuenta, detalle, nivel, sum(debe) as debe, sum(haber) as haber, sum(deudor) as deudor, sum(acreedor) as  acreedor, tipo_cuenta
 	                FROM  co_resumen_temp where '  .$where1.'
                    group by cuenta, detalle, nivel, tipo_cuenta
                    order by cuenta, detalle';
                ...
                $sql2 = 'SELECT cuenta, detalle, nivel, debe, haber, deudor, acreedor, anio, tipo_cuenta
 	                FROM  co_resumen_temp where '  .$where1;
            }
        ...
        echo '<table width="100%" border="0" cellpadding="0" cellspacing="0">
              <tr>
                  ...
                  <td  width="60%" rowspan="2" style="text-align: center"><b>'.$_SESSION['razon'].'</b><br>
                        ...
                        <b>BALANCE DE RESULTADOS POR UNIDAD EJECUTORA '.$f1.'  al '.$f2.'</b></td>
                  <td  width="20%">&nbsp;</td>
                </tr>
                <tr>
    ...
    //----------------------------------------------------------------------------------------
    function activos_tabla( $sql,$bandera,$nivel ){
        
        
        
        $stmta = $this->bd->ejecutar($sql);
        
        
        $ndebe1 = 0;
        
        while ($x=$this->bd->obtener_fila($stmta)){
            ...
            if ( $bandera == 1 ){
                ...
                $xx = $this->bd->query_array('co_resumen_temp',
                    ...
                    'anio='.$this->bd->sqlvalue_inyeccion($this->anio ,true). " and tipo_cuenta = 'I' and
         cuenta like ".$this->bd->sqlvalue_inyeccion($cuenta ,true)
            ...
            if ( $deudor <> 0 ){
                echo "<td  ".$nivel_color." align='right'>".number_format(abs($deudor),2)."</td>";
            }else {
                echo "<td>- </td>";
            }
        ...
        echo "<td align='right'></td>";
        echo "<td align='right'></td>";
        echo "<td align='right'><b>".number_format(abs($ndebe1),2)."</b></td>";
    ...
    //----------------------------------------------------------------------------------------
    function pasivo_tabla( $sql,$bandera,$nivel ){
        
        
        
        $stmtp = $this->bd->ejecutar($sql);
        
        
        $ndebe1 = 0;
        
        while ($x=$this->bd->obtener_fila($stmtp)){
            ...
            if ( $bandera == 1 ){
                ...
                $xx = $this->bd->query_array('co_resumen_temp',
                    'sum(debe) as debe, sum(haber) as haber,sum(deudor) as deudor,sum(acreedor) as acreedor',
                    'anio='.$this->bd->sqlvalue_inyeccion($this->anio ,true). " and ( tipo_cuenta = 'G' ) and
                     cuenta like ".$this->bd->sqlvalue_inyeccion($cuenta ,true)
                    );
...
if (isset($_POST["brfecha1"]))	{
    ...
    $gestion->grilla( $f1,$f2,$tipo,$nivel);
