...
class proceso{
	...
	function grilla( $f1,$f2,$com1){
	    ...
	    echo '<div class="col-md-2"> </div><div class="col-md-7" style="padding: 30px">  ';
	        ...
	        $this->cabecera('<b>VENTAS</b>',$com1);
	        $ingreso1= $this->Bloque_Activo($f1,$f2,1,1);
	 
	  
	    $valida = $this->Bloque_Activo_verificar($f1,$f2,2,1);
	    if ( $valida  <> 0 ){
	        $this->cabecera('<b>GASTO/COSTO</b>',$com1);
	        $ingreso2 = $this->Bloque_Activo($f1,$f2,2,1);
	    }
	    
	    $valida = $this->Bloque_Activo_verificar($f1,$f2,3,1);
	    if ( $valida  <> 0 ){
	        $this->cabecera('<b>TRANSFERENCIAS NETAS</b>',$com1);
	        $ingreso3 = $this->Bloque_Activo($f1,$f2,3,1);
	    }
	    
	    $valida = $this->Bloque_Activo_verificar($f1,$f2,4,1);
	    if ( $valida  <> 0 ){
	        $this->cabecera('<b>RESULTADO FINANCIERO</b>',$com1);
	        $ingreso4 = $this->Bloque_Activo($f1,$f2,4,1);
	    }
	   
	    $valida = $this->Bloque_Activo_verificar($f1,$f2,5,1);
	    if ( $valida  <> 0 ){
	...
	public function Bloque_Activo( $f1,$f2,$orden1,$orden2 ){
	    ...
	    $sql = 'SELECT    orden1, orden2, grupo1, grupo2, guia, cta1, cta2, cta3, sinsigno, consigno, anio
                    ...
                    where orden1 = '.$this->bd->sqlvalue_inyeccion( $orden1 ,true) .' order by orden2 ';
	...
	//-----------------
	public function Bloque_Activo_verificar( $f1,$f2,$orden1,$orden2 ){
...
//--------------------
	...
	function cabecera($titulo,$com1){
	    ...
	    echo '<table  class="table table-bordered" width="100%" style="font-size: 14px;table-layout: auto">';
 ...
 */
	...
	function cabecera_total($titulo,$saldo){
	    ...
	    echo '  <tr>
                    <td style="background-color:#d9edf7;padding: 5px;" width="80%"></b>'.$titulo.'</b></td>
                    <td style="background-color:#d9edf7;padding: 5px;" width="20%" align="right"><b>'.number_format($saldo,2).'</b></td>
                </tr></table>';
