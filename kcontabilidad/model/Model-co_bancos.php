...
class proceso{
	...
	function consultaId($accion,$id ){
	 ...
	 $qquery = array(
				array( campo => 'id_concilia',    valor =>$id,  filtro => 'S',   visor => 'S'),
	...
	function edicion($id){
	    ...
	    $this->ATabla[15][valor] =  $this->saldo_bancos_i($id,'deposito');
	...
	function saldo_bancos_e($id,$tipo){
	    ...
	    $a = $this->bd->query_array('co_conciliad','sum(haber) as egreso', 
	...
	function saldo_bancos_i($id,$tipo){
	    ...
	    $a = $this->bd->query_array('co_conciliad','sum(debe) as ingreso',
	        'id_concilia='.$this->bd->sqlvalue_inyeccion($id,true). " and
                                     tipo = ".$this->bd->sqlvalue_inyeccion($tipo,true)
	        );
