...
if ( $id > 0 ) {
                  ...
                  $xx = $bd->query_array('presupuesto.view_tram_inventario',   // TABLA
                  'count(*) as nn',                        // CAMPOS
                  'id_tramite='.$bd->sqlvalue_inyeccion($id,true) // CONDICION
                  );
                  
                  if ( $xx['nn'] > 0) { 

                        $valida =   BuscaActivo( $bd, $id);
                        ...
                        if ( $valida > 0 )    {
                            ...
                            if ( $valida == 1 )  {
                                ...
                                $numero3        = ' <img src="../../kimages/inventario_si.png" align="absmiddle" >';
                                $detalle        = $numero3.'<b>  Ingreso a inventarios Nro.Comprobante: '.trim($inv['comprobante']).' '. trim($inv['detalle']) .'</b>';
                                $completo1       = $bd->__user($inv['sesion']);
                                $sesion_nombre  = $completo1['completo'];
                                $fecha_texto3   =  $inv['fecha'] ;

                            } else {
                                $inv = $bd->query_array('activo.ac_bienes','factura,idproveedor,fecha_adquisicion,sesion',
                                'id_tramite='.$bd->sqlvalue_inyeccion($id,true)
                                );
                                $numero3        = ' <img src="../../kimages/inventario_si.png" align="absmiddle" >';
                                $detalle        = $numero3.'<b>  Ingreso a Activo Fijo  : '.trim($inv['factura']).'</b>';
                                $completo1       = $bd->__user($inv['sesion']);
                                $sesion_nombre  = $completo1['completo'];
                                $fecha_texto3   =  $inv['fecha_adquisicion'] ;
                            }
                              ...
                              flujo_tramite_det('',$detalle,$fecha_texto3,strtoupper($sesion_nombre));
                                ...
                                $numero3        = ' <img src="../../kimages/inventario_no.png" align="absmiddle" >';
                                $detalle        = $numero3.'<b> NO EXISTE INFORMACION INVENTARIOS/ACTIVOS FIJOS</b>' ;
                                $completo1       = 'USUARIO NO REGISTRO';
                                $sesion_nombre  = '-';
                                $fecha_texto3   =  '-';
                                flujo_tramite_det('',$detalle,$fecha_texto3,strtoupper($sesion_nombre));
                            }  
...
//-------------------------------------------------------------------------------------
function BuscaActivo( $bd, $qtramite){
    ...
    $y = $bd->query_array('activo.ac_bienes',   // TABLA
    'count(*) as nn',  
    "id_tramite=".$bd->sqlvalue_inyeccion( $qtramite ,true) // CONDICION
 );
