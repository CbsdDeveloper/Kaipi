...
class proceso{
	...
	function arregla_retencion($id){
	    ...
	    if ($longitud > 5 ) {
	        ...
	        if ($longitudProve > 5 ) {
	                ...
	                $result = $this->div_resultado('editar',$id,1,'aprobado');
	            
	        }else {
	            
	            
	            $result = '<img src="../../kimages/nogusta.png" align="absmiddle"/>&nbsp;<b>INGRESE PROVEEDOR</b>';
	            
	        }
	    }else {
	        
	        
	        $result = '<img src="../../kimages/nogusta.png" align="absmiddle"/>&nbsp;<b>INGRESE LA CUENTA DEL GASTO PARA GUARDAR LA TRANSACCION</b>';
            		...
            		if (trim($estado) == 'digitado'){
            		 			...
            		 			$sql = "UPDATE co_asiento
            							    SET 	detalle      =".$this->bd->sqlvalue_inyeccion(trim(@$_POST["detalle"]), true).",
                                                    apagar       =".$this->bd->sqlvalue_inyeccion($total_apagar, true).",
                                                    id_periodo   =".$this->bd->sqlvalue_inyeccion($id_periodo, true).",
            								        idprov       =".$this->bd->sqlvalue_inyeccion(trim($idprov), true).",
            										cuentag      =".$this->bd->sqlvalue_inyeccion(trim($cuenta), true).",
            										documento    =".$this->bd->sqlvalue_inyeccion(trim($secuencial), true)."
            					...
            					if ( !empty($idventas)) {
            					    ...
            					    $this->bd->_UpdateSQL('co_ventas',$this->ATabla,$idventas); 
            					}
            					else 	{
    	 	        ...
    	 	        $result = '<img src="../../kimages/nogusta.png" align="absmiddle"/>&nbsp;<b>ASIENTO APROBADO NO SE PUEDE EDITAR</b>';
	...
	function eliminar($id ){
	    ...
	    $this->saldos->_elimina_asientocxc($id,$estado);
