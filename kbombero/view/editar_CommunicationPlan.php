...
if (isset($_GET['id'])) {
    $id = $_GET['id'];
    $plan = $controller->getPlanById($id);
    if (!$plan) {
...
<head>
    <meta charset="UTF-8">
    <title>Editar Plan de Comunicación</title>
...
<body>
    ...
    <div class="content">
        <div class="cuadro">
            <h6 class="card-header">Editar Plan de Comunicación</h6><br>
            <form id="editarPlanForm">
             <!-- Fechas de creacion y edicion -->
             <div class="edit-info mb-4">
                    <div class="edit-info-cuadro">
                        <i class="fas fa-plus-square"></i>
                        <span id="created-info">Creado: <?php echo $plan['fecha_creacion_formatted']; ?></span>
                    </div>
                    <div class="edit-info-cuadro">
                        <i class="fas fa-edit"></i>
                        <span id="edited-info">Última edición: <?php echo $plan['fecha_edicion_formatted']; ?></span>
                    </div>
                </div>

                <input type="hidden" id="id_incidente" name="id_incidente" value="<?php echo $_GET['id_incidente'] ?? ''; ?>">
                <input type="hidden" name="id" value="<?= htmlspecialchars($plan['id_plan_comunicaciones']) ?>">
                ...
                <div class="row">
                    <div class="col-xl-6 col-md-6 col-sm-12">
                        ...
                        <select class="form-select" id="periodo" name="periodo" required>
                            ...
                            <option value="Diario" <?= $plan['periodo'] == 'Diario' ? 'selected' : '' ?>>Diario</option>
                            <option value="Semanal" <?= $plan['periodo'] == 'Semanal' ? 'selected' : '' ?>>Semanal</option>
                    ...
                    <div class="col-xl-6 col-md-6 col-sm-12">
                        ...
                        <select class="form-select" id="sistema" name="sistema" required>
                            <option value="">Seleccione...</option>
                            <option value="Radio" <?= $plan['sistema'] == 'Radio' ? 'selected' : '' ?>>Radio</option>
                            <option value="Teléfono" <?= $plan['sistema'] == 'Teléfono' ? 'selected' : '' ?>>Teléfono</option>
                            <option value="Satélite" <?= $plan['sistema'] == 'Satélite' ? 'selected' : '' ?>>Satélite</option>
                        </select>
                    </div>
                    <div class="col-xl-6 col-md-6 col-sm-12">
                        <label for="canal_frecuencia" class="form-label">Canal/Frecuencia:</label>
                        <input type="text" class="form-control" id="canal_frecuencia" name="canal_frecuencia" value="<?= htmlspecialchars($plan['canal_frecuencia']) ?>" required>
                    </div>
                    <div class="col-xl-6 col-md-6 col-sm-12">
                        <label for="asignado" class="form-label">Asignado:</label>
                        <input type="text" class="form-control" id="asignado" name="asignado" value="<?= htmlspecialchars($plan['asignado']) ?>" required>
                    </div>
                    <div class="col-xl-12 col-md-12 col-sm-12">
                        <label for="ubicacion" class="form-label">Ubicación:</label>
                        <input type="text" class="form-control" id="ubicacion" name="ubicacion" value="<?= htmlspecialchars($plan['ubicacion']) ?>" required>
                    </div>
                    <div class="col-xl-12 col-md-12 col-sm-12">
                        <label for="observaciones" class="form-label">Observaciones:</label>
                        <textarea class="form-control" id="observaciones" name="observaciones" rows="3"><?= htmlspecialchars($plan['observaciones']) ?></textarea>
                    </div>
                    <div class="col-xl-6 col-md-6 col-sm-12">
                        <label for="elaborado_por" class="form-label">Elaborado por:</label>
                        <input type="text" class="form-control" id="elaborado_por" name="elaborado_por" value="<?= htmlspecialchars($plan['elaborado_por']) ?>" required>
                    </div>
                    <div class="col-xl-6 col-md-6 col-sm-12">
                ...
                <div class="form-buttons">
                    <a href="listar_CommunicationPlan.php?id_incidente=<?php echo htmlspecialchars($id_incidente); ?>" class="btn btn-secondary">Regresar</a>
    ...
    <script>
        $(document).ready(function() {
            $('#editarPlanForm').submit(function(e) {
                e.preventDefault();
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success) {
                            ...
                            window.location.href = 'listar_CommunicationPlan.php?id_incidente=' + id_incidente;
                        } else {
