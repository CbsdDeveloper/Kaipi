...
session_start();   
    ...
    class Model_tarea_seg01 {
      ...
      function Model_tarea_seg01 ( ){
                ...
                $this->ATabla = array(
                    ...
                    array( campo => 'idpac',tipo => 'NUMBER',id => '2',add => 'S', edit => 'S', valor => '-', key => 'N'),
                    ...
                    array( campo => 'seg_solicitado',tipo => 'NUMBER',id => '14',add => 'S', edit => 'S', valor => '-', key => 'N'),
      ...
      function div_resultado($accion,$id,$tipo){
            //inicializamos la clase para conectarnos a la bd
             ...
             if ($tipo == 1){
                 ...
                 $resultado = '<div class="alert alert-info"><img src="../../kimages/ksavee.png" align="absmiddle"/>&nbsp;<b>INFORMACION GUARDADA CON EXITO ['.$id.']... VERIFIQUE EL AVANCE DE SU PROCESO...</b></div>';
 ...
 function consultaId($accion,$id ){
     ...
     function agregar(   ){
         ...
         $seg_comentario    = trim($_POST["seg_tarea_seg"]);
         
         $modulo            = trim($_POST["seg_proceso"]);
         
         $seg_solicitado    = $_POST["seg_solicitado"];
         
         $saldo_tarea       = $_POST["saldo_tarea"];
          
         
         $valida_mes = explode('-',$seg_fecha);
         
         $mes_actual = $valida_mes[1];
         
         $datos2 = $this->bd->query_array(
             'planificacion.sitareadetalle',
             'mes, inicial',
             'inicial > 0 and 
              idtarea='.$this->bd->sqlvalue_inyeccion( $tarea ,true).' and 
              mes='.$this->bd->sqlvalue_inyeccion( $mes_actual ,true)
             );
         
         
        $monto_mes = $datos2['inicial'];
         ...
         if( $seg_solicitado > 0   ){
                    ...
                    if( $seg_solicitado >   $saldo_tarea   ){
                
                        $result = '<img src="../../kimages/kdel.png" align="absmiddle" />&nbsp;<b>SALDO EXCEDE DE LO SOLICITADO ?</b>';
 
                }	else {
                   
                    if( $seg_solicitado >   $monto_mes   ){
                        
                        $result = '<img src="../../kimages/kdel.png" align="absmiddle" />&nbsp;<b>SALDO EXCEDE A LO PLANIFICADO DEL MES!!! '.$monto_mes .'</b>';
                    }
                    else     {
