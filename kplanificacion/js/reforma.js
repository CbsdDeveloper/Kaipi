...
$(document).ready(function(){
        ...
        oTableIngreso = $('#jsontable_ingreso').dataTable(); 
    	 ...
    	 var parametros = {
	 			       ...
	 			       'accion' : accion
						  ...
						  $.ajax({
										...
										url:   '../model/Model-reforma_detalle.php',
...
//-----------------------------------------------------------------
function goToURLReforma(tipo,partida,saldo ) {
...
function LimpiarPantalla() {
		...
		var parametros = {
	  	...
	  	$.ajax({
	 			data:  parametros,
	 			url:   '../model/ajax_DetalleReforma.php',
	 			type:  'GET' ,
	 			cache: false,
	 			beforeSend: function () { 
...
function BusquedaGrilla(oTable){        	 
	  ...
	  var ftipo = $("#ftipo").val();
	  var ftipo_reforma = $("#ftipo_reforma").val();
...
function FormView()
{
	 ...
	 $("#ViewForm").load('../controller/Controller-reforma.php');
...
function BusquedaGrillaGasto(oTableGasto){        	 
		...
		$.ajax({
		    ...
		    url: '../grilla/grilla_reforma_gasto.php',
			dataType: 'json',
			cache: false,
			...
			if(s){
				...
				for(var i = 0; i < s.length; i++) {
					...
					oTableGasto.fnAddData([
						 ...
						 s[i][6],
						 s[i][7],
	                     '<button class="btn btn-xs" onClick="javascript:goToURLReforma('+"'gasto'"+','+ "'" + s[i][0] +"'," + s[i][5] +')"><i class="glyphicon glyphicon-edit"></i></button> '  
...
//-------------------------------
function BusquedaGrillaIngreso(oTableIngreso){        	 


	$.ajax({

	    url: '../grilla/grilla_reforma_ingreso.php',
		dataType: 'json',
		cache: false,
		...
		if(s){
			...
			for(var i = 0; i < s.length; i++) {
				...
				oTableIngreso.fnAddData([
					 ...
					 s[i][6],
					 s[i][7],
                     '<button class="btn btn-xs" onClick="javascript:goToURLReforma('+"'ingreso'"+','+ "'" + s[i][0] +"'," + s[i][5] +')"><i class="glyphicon glyphicon-edit"></i></button> '  
...
function FormFiltro()
{
	 ...
	 $("#ViewFiltro").load('../controller/Controller-reforma_filtro.php');
...
function PonePartida( ){
    ...
    if ( tipo == 'G'){
   	 ...
   	 BusquedaGrillaGasto(oTableGasto);
   	 $('#myModalIngreso').modal('hide');
   	 ...
   	 BusquedaGrillaIngreso(oTableIngreso);
...
function actualiza_datod(saldo,valor,id,reforma)
 {
	 ...
	 if (tipo_reforma == 'Traspaso'){
		  	 ...
		  	 if (valor > saldo ) {
		  		   alert('Saldo supera al valor del saldo');
