 ...
 <?php
    ...
    class componente
    {
        ...
        function Formulario()
        {
            ...
            $array = $this->bd->__user(str_replace('@cbsd.gob.ec', '', $this->sesion));
            ...
            $resultadob = $this->bd->ejecutar("select idbodega as codigo, trim(nombre) as nombre from view_bodega 
            where registro =" . $this->bd->sqlvalue_inyeccion($this->ruc, true) . " and activo=" . $this->bd->sqlvalue_inyeccion('S', true) . " and idbodega=1 order by 1");
            ...
            $this->obj->text->text('Fecha', 'date', 'fecha', 10, 10, $datos, 'required', 'readonly', 'div-2-4');
            ...
            echo '<div class="col-md-3"  style="padding-bottom: 10px;padding-top: 10px">';
            $evento = ' onClick="ListarProductos()" ';
            $cboton2 = ' <a href="#" class="btn btn-warning btn-sm" role="button" ' . $evento . ' > <span class="glyphicon glyphicon-search"></span> Buscar Productos</a>';
            echo  $cboton2;
            echo '	</div>';
 ...
 <script type="text/javascript">
     ...
     jQuery('#articulo').typeahead({
         ...
         source: function(query, process) {
             ...
             return $.get('../../kinventario/model/AutoCompleteProdSolicitudBodega.php', {
                 query: query
             }, function(data) {
                 console.log(data);
                 data = $.parseJSON(data);
