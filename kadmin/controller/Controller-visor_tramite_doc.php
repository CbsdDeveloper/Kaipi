...
class proceso{
    ...
    function GrillaGasto_periodo_uno(  $ccedula,$cadena,$ccaso){
        ...
        if ($len >= 3 ){
            $solicita_where = '';
            $ruc_where      = 'idprov ='. $this->bd->sqlvalue_inyeccion(trim($ccedula) , true)  ;
        }else{
            $ruc_where      = '';
            if ( $len1  >= 6){
                $solicita_where =  'nombre_solicita like'. $this->bd->sqlvalue_inyeccion(trim($cadena) , true)  ;
            }else{
                $solicita_where =  'nombre_solicita ='. $this->bd->sqlvalue_inyeccion(trim($cadena) , true)  ;
            }
        }
        ...
        if ($ccaso > 0 ){
            ...
            $solicita_where = '';
            $ruc_where      = ' idcaso ='. $this->bd->sqlvalue_inyeccion($ccaso , true)  ;
            ...
            $x = $this->bd->query_array('flow.view_proceso_caso',   // TABLA
                ...
                'idcaso='.$this->bd->sqlvalue_inyeccion($ccaso,true) // CONDICION
                );
        ...
        $sql = 'SELECT  idcaso,  fecha,
                        departamento_caso as unidad,nombre_solicita, estado, proceso,caso, ambito, tarea,  dias_trascurrido
    ...
    function grilla($resultado,$tipo,$presupuesto,$nombre)  {
        ...
        echo '<th width="20%" bgcolor="#167cd8" style="color: #F4F4F4">Tramite</th>';
        echo '<th width="20%" bgcolor="#167cd8" style="color: #F4F4F4">Caso</th>';
        echo '<th width="25%" bgcolor="#167cd8" style="color: #F4F4F4">Tarea</th>';
        echo '<th width="5%" bgcolor="#167cd8" style="color: #F4F4F4">Dias</th>';
        ...
        while($row=pg_fetch_assoc($resultado)) {
            ...
            echo "<td><a ".$referencia." ><b>".trim($row['idcaso']).'</b></a></td>';
            echo "<td>".trim($row['fecha']).'</td>';
            echo "<td>".trim($row['unidad']).'</td>';
            echo "<td>".trim($row['proceso']).'</td>';
            echo "<td>".trim($row['caso']).'</td>';
            echo "<td>".trim($row['tarea']).'</td>';
             echo "<td>".trim($row['dias_trascurrido']).'</td>';
