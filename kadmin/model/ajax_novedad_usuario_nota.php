...
$fechah          = $_GET['fechah'];   // VARIABLE DE ENTRADA CODIGO DE BITACORA
...
$sql = "select desde || ' ' || hora_desde as desde, hasta ||' ' || hora_saluda as hora,destino,mo_salida,observaciones,registro
        FROM  hoja_ruta
            where registro  between ".$bd->sqlvalue_inyeccion($fecha, true). " and ".$bd->sqlvalue_inyeccion($fechah, true). " AND 
...
$cabecera =  "DESDE,HASTA,DESTINO,MOTIVO DE SALIDA,OBSERVACIONES,REGISTRO"; // CABECERA DE TABLAR GRILLA HA VISUALIZAR


$tema       = '<h3><b>HOJA DE RUTA POR PERIODO</b></h3>';
$titulo_ref = '<b>'.$completo.'<br>'.$hoy .'</b>';

titulo($obj,$sesion,$unidad ,$tema,$titulo_ref);

echo '<h4><b>'.$datos_titulo['completo'] .'<br>'.$datos_titulo['cargo'].'<br>'.$datos_titulo['cedula'].'<br>Periodo:'.$fecha.' a '.$fechah.'</h4></b>';
