...
<?php
 ...
 ?>
<body>
 <form action="editor_pie?action=save&id=<?php echo $_GET['id']; ?>" method="post" enctype="application/x-www-form-urlencoded" id="formulario">	 
...
<?php
 ...
 function K_consulta($id ,$obj,$bd ){
 	 ...
 	 $sql = "SELECT pie as contenido
	  FROM par_reporte  where idreporte_sis =".$bd->sqlvalue_inyeccion($id ,true);
 ...
 function K_editar($id, $obj,$bd ){
 	  ...
 	  $sql = "UPDATE par_reporte
			   SET  pie=".$bd->sqlvalue_inyeccion(@$_POST["editor1"], true)."
			 WHERE idreporte_sis =".$bd->sqlvalue_inyeccion($id, true);
