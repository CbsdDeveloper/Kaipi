...
$.ajax({
		data:  parametros,
		url:   '../model/Model-ad_combustible_in.php',
		type:  'GET' ,
 ...
 function openFile(url,ancho,alto) {
  ...
  function BusquedaGrilla(oTable){        
	      ...
	      var parametros = {
			...
			$.ajax({
			 	data:  parametros,  
			    url: '../grilla/grilla_ad_combustible_in.php',
				dataType: 'json',
				success: function(s){
						//console.log(s); 
						oTable.fnClearTable();
							...
							for(var i = 0; i < s.length; i++) {
							 oTable.fnAddData([
								...
								s[i][2],
								   ...
								   s[i][7],
								   s[i][8],
                                	'<button class="btn btn-xs" title="Visualizar Combustible" onClick="javascript:goToURL('+"'editar'"+','+   s[i][0]   +')"><i class="glyphicon glyphicon-time"></i></button>&nbsp;'  +
									'<button class="btn btn-xs" title="Eliminar Combustible" onClick="javascript:goToURL('+"'del'"+','+   s[i][0]   +')"><i class="glyphicon glyphicon-remove"></i></button>'
								]);										
						} // End For
				  }						
				},
...
function FormView()
{
	 ...
	 $("#ViewForm").load('../controller/Controller-ad_combustible_interno.php');
