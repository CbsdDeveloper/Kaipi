...
function chat_msg( $bd, $obj ){
    ...
    $sql = "SELECT  a.sesion,  a.fecha, a.mensaje,  b.url ,b.nombre,b.tipourl
              FROM web_chat_directo a, par_usuario b
              where a.estado = 'E' and
                    a.registro = ".$bd->sqlvalue_inyeccion($registro, true)." and
                    a.modulo = 'panel' and
                    trim(b.login) = trim(a.sesion)
              order by a.id_chat desc limit 8" ;
    ...
    while ($x=$bd->obtener_fila($stmt)){
        ...
        $result = $result.$div_estilo.'
                      ...
                      <img class="direct-chat-img" src="'.$carpeta.$x['url'].'" title="message user image"  width="25" height="25"/>
					  <!-- /.direct-chat-img -->
