...
<h4>Importando archivo CSV</h4>
  <form action='Model_carga_inicial_excel.php' method='post' enctype="multipart/form-data">
    ...
    <input type="text" required  id="movi" name="movi" placeholder="Nro. Movimiento">
...
if(isset($_POST['submit']))
{
    ...
    //---- secuencia	producto	referencia	categoria	unidad	cantidad	facturacion	tributo	costo	stock	precio_iva
        ...
        while (($data = fgetcsv($handle, 1000, ";")) !== FALSE)
        {
            ...
            $id_producto = _busca_producto($bd,$nombre_producto,$ruc_registro);
                          ...
                          if ( empty($id_producto)) {
                                     ...
                                     if ($lon1  > 5 ) {
                                         ...
                                         $id_producto = _guarda_producto($bd,$nombre_producto ,
                                            ...
                                            $facturacion,$tributo,$costo,$stock,$precio_iva,$ruc_registro);
                                         // precio
                                         if ( $id_producto > 0) {
                                            
                                             _busca_precio($bd	,$id_producto,$precio_iva);
                                               
                                             _guarda_detalle($bd, $id_producto ,$costo, $stock,$ruc_registro );
...
//---------------------
    ...
    function _guarda_detalle($bd,$id_producto ,$costo,$stock ){
        ...
        $Existe = $bd->query_array(
            'inv_movimiento_det',
            'count(*) as nn ',
            'idproducto='.$bd->sqlvalue_inyeccion($id_producto,true) . ' and id_movimiento = -1'
            );
        ...
        {
            if ($cantidad > 0  ){
                $DatosCarga = nuevo( $id_movimiento,$id_producto, $bd ,$estado,$tipo,$costo,$cantidad);
            }
  ...
  //----------------------------------------
  //--- funciones grud
    function nuevo($id_movimiento,$id_producto, $bd ,$estado,$tipo,$costo,$cantidad){
        ...
        'costo,tributo,saldo',
        'idproducto='.$bd->sqlvalue_inyeccion($id_producto,true)
        );
