	...
	var parametros = {
	  ...
	  $.ajax({
					...
					success:  function (data) {
							 ...
							 BusquedaConta(id);
							 BusquedaGrillaItems(oTableItems,id)
  					} 
			}); 
...
//---------------------------------
//---------
function goToProcesoFin() {

  	   var id_importacion = $("#id_importacion").val();
  	   
	   var total = 			parseFloat($("#total_gasto_resumen").val());   
	   ...
	   if ( total >  0 ) {
		      ...
		      $.ajax({
						data:  parametros,
						url:   '../model/ajax_marca_gastof.php',
						type:  'GET' ,
						cache: false,
						 
						success:  function (data) {
								 $("#DistribuyeGasto").html(data);   
		
								 BusquedaGrillaItems(oTableItems,id_importacion);
		   ...
		   alert('Seleccion el monto a distribuir ');
		   
	   }
		 
} 
//-----------------
function ResumenTotalIMportacion() {
	   ...
	   var id_importacion = $("#id_importacion").val();
        ...
        var parametros1 = {
		  ...
		  $.ajax({
						data:  parametros,
						url:   '../model/ajax_importacion_resumen.php',
						type:  'GET' ,
						cache: false,
...
function myFunction(codigo,objeto)
{
      ...
      $.ajax({
				data:  parametros,
				url:   '../model/ajax_marca_gasto.php',
				type:  'GET' ,
				cache: false,
				 
				success:  function (data) {
						 $("#total_gasto_resumen").val(data);   
...
//------------------------------------------------------------------------- 
 ...
 //----------
  //------------------------------------------------------------------------- 
  function BusquedaGrillaItems(oTableItems,id){        	 
    ...
    var parametros = {
		...
		$.ajax({
		 	data:  parametros,
 		    url: '../grilla/grilla_inv_importaciones_items_all.php'  ,
			dataType: 'json',
			cache: false,
			success: function(s){
			 console.log(s); 
  ...
  function BusquedaConta( id) {
		...
		var id_importacion = $("#id_importacion").val();
		 ...
		 if (cierre == 'S') {
			 ...
			 $.ajax({
				data:  parametros,
				url:   '../model/Model_listaImpCxp.php',
				type:  'GET' ,
				cache: false,
 
				success:  function (data) {
						 $("#DetalleGasto").html(data);  // $("#cuenta").html(response);
 ...
 function modulo()
 {
 	 var modulo_dato =  'kinventario';
 	 
 	 var parametros = {
			    'ViewModulo' : modulo_dato 
    };
 	$.ajax({
			data:  parametros,
...
//-----------------
 function FormView()
 {
	 ...
	 $("#ViewForm").load('../controller/Controller-' + formulario +'_li');
...
//----------------------
 function FormFiltro()
 {
	 ...
	 $("#ViewFiltro").load('../controller/Controller-'+formulario+'_filtro_li.php');
