        ...
        $("#fechae").val(fecha);
		 ...
		 $('#GuardarSeg').on('click',function(){
		  ...
		  $('#loadxls').on('click',function(){
				...
				var cadena = 'servicio='+frubro ;

				var page = "../reportes/excelServicio.php?"+cadena;  
...
//----------------10,2,1575,'E','2021-01-01'
function Elimina_pre_emision(a,b,c,d,e) {
}
//----------------------------
...
function FormView()
{
	 ...
	 $("#ViewForm").load('../controller/Controller-ren_tramites_uni');
	 
	 $("#ViewFormOpcion").load('../controller/Controller-ren_tramite_boton.php');
	 
	 $("#ViewRubro").load('../controller/Controller-ren_tramite_lista_uni.php');
...
function Abrir_informacion(  )
{
	
	$('#myModalabrir').modal('show');
...
function abrirTramite(  )
{
	...
	alertify.confirm("<p>Desea Aperturar/Abrir del tramite seleccionado<br><br></p>", function (e) {
		if (e) {
			...
			$.ajax({
				...
				success: function(response)
				{
					...
					$("#result").html('<img src="../../kimages/z_add.png" align="absmiddle"/><b> APERTURA DE TRANSACCION... </b>'+response);
				}
			});


			$('#myModalabrir').modal('hide'); 
		}
	   }); 
