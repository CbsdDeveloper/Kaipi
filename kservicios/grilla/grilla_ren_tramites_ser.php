    ...
    class proceso{
      ...
      public function BusquedaGrilla($GET){
        ...
        $cadena1 = '( id_rubro ='.$this->bd->sqlvalue_inyeccion(trim($frubro),true).") and  ";
      	...
      	if ( $longitud1 > 3 ){
      	    $cadena0 =  " estado <>  'cierre' and ( idprov like ".$this->bd->sqlvalue_inyeccion(trim($ruc).'%',true).')   ';
      	    $where = $cadena1.$cadena0;
      	}
      	
      	if ( $longitud2 > 3 ){
      	    $cadena0 =  " estado <>  'cierre' and ( razon like ".$this->bd->sqlvalue_inyeccion(trim($nombrec).'%',true).')  ';
      	    $where = $cadena1.$cadena0;
      	}
      	
      	
      	$sql = 'SELECT id_ren_tramite,fecha_inicio, idprov,razon, detalle,  login   ,usuario
      	...
      	while ($fetch=$this->bd->obtener_fila($resultado)){
      	    ...
      	    $detalle =  trim($fetch['detalle']).' '.trim($fetch['usuario']);
