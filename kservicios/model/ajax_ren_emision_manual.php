...
class proceso
{
    ...
    {
        $monto = '0.00';
        $orden_total = $x_tramite['orden_monto'];
        $base_iva = $x_tramite['orden_monto'] / 1.15;
        $monto_iva = $base_iva  * 0.15;

        $this->ATabla_detalle[0]["valor"]  = $idproducto_ser;

        $this->ATabla_detalle[1]["valor"]  =  $id_movimiento;

        $this->ATabla_detalle[3]["valor"]   =   $base_iva;
        $this->ATabla_detalle[4]["valor"]   =   $monto_iva;

        $this->ATabla_detalle[5]["valor"]  =   $base_iva;
    ...
    public function Emision_titulos($GET)
    {
        ...
        // $x_tramite = $this->bd_externo->query_array(
        //     'recaudacion.tb_ordenescobro',   // TABLA
        //     '*',                        // CAMPOS
        ...
        $fecha_orden  =  date("Y-m-d"); // trim($x_tramite['orden_registro']);
        ...
        $orden_entidad       = 'capacitaciones'; // trim($x_tramite['orden_entidad']);
        ...
        if ($orden_entidad == 'capacitaciones') {
            $id_rubro = 11;
            $idproducto_ser = 15;
        }
        ...
        $this->ATabla[8]["valor"]   =  $monto_iva;
        $this->ATabla[9]["valor"]   =  $monto;
        $this->ATabla[10]["valor"]  =  $base_iva;
        $this->ATabla[11]["valor"]  =  $monto;
        $this->ATabla[12]["valor"]  =  $monto;
        $this->ATabla[13]["valor"]  =  $monto;

        $this->ATabla[33]["valor"]  =    $base_iva;
        ...
        $x =   $this->bd->query_array(
            'par_ciu',   // TABLA
            'id_par_ciu, idprov, razon, direccion, correo',                        // CAMPOS
            'id_par_ciu=' .  $this->bd->sqlvalue_inyeccion($id_par_ciu, true) // CONDICION
        );

        echo json_encode(array("a" => trim($id_par_ciu), "idprov" => trim($x["idprov"]), "razon" => trim($x["razon"]), "direccion" => trim($x["direccion"]), "correo" => trim($x["correo"])));
    }
    //-------------------------------------------------------------  
    ...
    */
    public function agregar($idprov, $fila)
    {
        ...
        $nombre_1  =  $fila["orden_cliente_nombres"];
        $nombre_2  =  $fila["orden_cliente_apellidos"];
        ...
        $pro_direccion   =  $this->reemplazar(substr(trim($fila["orden_direccion"]), 0, 150));

        $contacto   =  $nombre_1 . ' ' .$nombre_2 . ' ' .$this->reemplazar($fila["orden_cliente_nombre"]);
