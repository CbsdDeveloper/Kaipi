<?php 
    ...
    class componente{
      ...
      function FiltroFormulario(){
        ...
        $datos['fecha1'] =  date("Y-m-d");
         ...
         $MATRIZ_SN = array(
      	...
      	$this->obj->list->lista('Estado',$MATRIZ_SN,'estado1',$datos,'','','div-2-4');
      ...
      function ListaDB( $titulo){
          ...
          if ( trim($ACaja['supervisor']) == 'S'){
              $resultado = $this->bd->ejecutar("select email as codigo, completo as nombre
			                     from par_usuario
								where caja = 'S' and estado = 'S' order by 2 asc");
          }else{
              $resultado = $this->bd->ejecutar("select email as codigo, completo as nombre
			                     from par_usuario
								where email = " .$this->bd->sqlvalue_inyeccion($this->sesion,true).' order by 2 asc');
          }
