...
class proceso{
	...
	function proceso( ){
		...
		$this->ATabla = array(
		        ...
		        array( campo => 'idsede',   tipo => 'NUMBER',   id => '15',  add => 'S',   edit => 'N',   valor => '-',   filtro => 'N',   key => 'N'),
		        array( campo => 'id_departamento',   tipo => 'NUMBER',   id => '16',  add => 'S',   edit => 'S',   valor => '-',   filtro => 'N',   key => 'N')
 		);
		
 		
		$this->tabla 	  	  = 'activo.ac_compra';
		
		$this->secuencia 	     = 'activo.ac_compra_id_movimiento_seq';
	...
	function consultaId($accion,$id ){
		...
		$qquery = array(  
		 ...
		 $this->bd->JqueryArrayVisor('activo.view_movimientoa',$qquery );
	...
	function eliminar($id ){
	    ...
	    if (trim($estado) == 'digitado') {
	        ...
	        $sql = 'delete from activo.ac_compra  where id_movimiento='.$this->bd->sqlvalue_inyeccion($id, true);
	        $this->bd->ejecutar($sql);
	        
	        
	        $sql = 'delete from  activo.ac_compra_det  where id_movimiento='.$this->bd->sqlvalue_inyeccion($id, true);
	        $this->bd->ejecutar($sql);
	...
	function AprobarComprobante($accion,$id,$tipo){
	    ...
	    $comprobante = $this->K_comprobante($tipo);
	        ...
	        $sql = "delete from activo.ac_compra 
	        ...
	        $sql = "delete from activo.ac_compra_det
 						 WHERE id_movimiento is null and sesion=".$this->bd->sqlvalue_inyeccion(trim($this->sesion) , true);
...
//----------------------------
  ...
  //------------------------------
	...
	function K_kardex($id,$tipo){
	   ...
	   //     $id_movimientod    = $x['id_movimientod'];
	        ...
	        if ($tipo == 'I'){
	            ...
	            $costo_total = (($costo * $cantidad)) + ($xa['costo'] *  $xa['saldo'] ) ;
	            
	            $costo_uni =  $costo_total / ($cantidad + $xa['saldo'] ) ;
    ...
    //--------------------
    	...
    	$sql = " UPDATE activo.ac_compra_det 
