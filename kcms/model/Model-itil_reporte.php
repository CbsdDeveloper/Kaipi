...
class proceso{
    ...
    function GrillaGasto_periodo_uno( $ffecha1,$ffecha2,$fmodulo,$vunidad){
        ...
        if ( $vunidad == '0'){
             ...
             $fuente_where = '   unidad ='. $this->bd->sqlvalue_inyeccion($vunidad , true) .' and ';
         }
        ...
        $sql = 'SELECT  id_tiket,  fecha,
                        unidad,nombre, estado, categoria,asunto, prioridad, novedad,  tecnico
            ...
            $this->titulo('GESTION SOPORTE TECNICO',$ffecha1,$ffecha2);
    ...
    function grilla($resultado,$tipo,$presupuesto,$nombre)  {
        ...
        echo '<th width="8%" bgcolor="#167cd8" style="color: #F4F4F4">Estado</th>';
        echo '<th width="5%" bgcolor="#167cd8" style="color: #F4F4F4">Categoria</th>';
        echo '<th width="15%" bgcolor="#167cd8" style="color: #F4F4F4">Asunto</th>';
        echo '<th width="22%" bgcolor="#167cd8" style="color: #F4F4F4">Novedad</th>';
        echo '<th width="15%" bgcolor="#167cd8" style="color: #F4F4F4">Tecnico</th>';
        ...
        while($row=pg_fetch_assoc($resultado)) {
            ...
            if (trim($row['estado'])== 'cerrado') {
                
                $cadena= '<a href="#"><img src="../../kimages/star_medio.png" align="absmiddle" /></a> ';
                
            }else {
                
                $cadena= '<a href="#"><img src="../../kimages/star.png" align="absmiddle" /></a> ';
            ...
            $evento = 'CargaDatos('.$row['id_tiket'].')';
            
            $referencia = ' href="#" onClick="'.$evento.'" data-toggle="modal" data-target="#myModalProducto" ';
            
            echo "<td><a ".$referencia." >".trim($row['id_tiket']).'</a> '.'</td>';
            echo "<td>".trim($row['fecha']).'</td>';
            echo "<td>".trim($row['unidad']).'</td>';
            echo "<td>".trim($row['nombre']).'</td>';
            echo "<td>".$cadena.trim($row['estado']).'</td>';
            echo "<td>".trim($row['categoria']).'</td>';
            echo "<td>".trim($row['asunto']).'</td>';
            echo "<td>".trim($row['novedad']).'</td>';
            echo "<td>".trim($row['tecnico']).'</td>';
        ...
        echo '<table width="100%" border="0" cellpadding="0" cellspacing="0" style="font-size: 13px;table-layout: auto">
              <tr>
                  ...
                  <td  width="60%" rowspan="2" style="text-align: center"><b>'.$_SESSION['razon'].'</b><br>
                         <br>
                        <b>GESTION SOPORTE TECNICO PERIODO '.$ffecha1.' AL '.$ffecha2.'  </b><br>
                         </b></td>
                  <td  width="20%">&nbsp;</td>
                </tr>
                <tr>
