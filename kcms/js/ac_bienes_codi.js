...
//------------------------------------------------------------------------- 
  function BusquedaGrilla(oTable){       
    ...
    var parametros = {
		...
		$.ajax({
		 	data:  parametros,
 		    url: '../grilla/grilla_ac_bienes.php',
			dataType: 'json',
			cache: false,
			success: function(s){
		    //console.log(s); 
			...
			for(var i = 0; i < s.length; i++) {
				 oTable.fnAddData([
						...
						s[i][5],
						s[i][6],
  					'<button title="Genere codigo de Barra" class="btn btn-xs" onClick="javascript:goToURL('+"'barra'"+','+ s[i][0] +')"><i class="glyphicon glyphicon-barcode"></i></button>&nbsp;' 
				]);										
			} // End For
		  } 						
		 } 
...
//-----------------
 function FormView()
 {
	 ...
	 $("#ViewForma").load('../controller/Controller-' + formulario);
