...
class componente{
    ...
    function _05_Cabecera_FacturaElectronica( $id){
        ...
        $ADatos = $this->bd->query_array(
            'web_registro',
            'razon, contacto, correo,direccion,felectronica',
            'ruc_registro='.$this->bd->sqlvalue_inyeccion( $this->ruc,true)
            );
        ...
        $﻿ATabla = array(
            ...
            array( campo => 'estab',   tipo => 'VARCHAR2',   id => '2',  add => 'S',   edit => 'S',   valor => '003',   filtro => 'N',   key => 'N'),
        ...
        $this->_10_Cabecera_Adicional( $cab_codigo,$this->Array_Cabecera['correo']);
    ...
    function _08_Cabecera_FacturaElectronica_Impuesto( $codFac){
        ...
        if ( $this->ArrayTotal['monto_iva'] > 0 ){
            ...
            $﻿ATablaRefIVA = array(
                ...
                array( campo => 'descuentoadicional',   tipo => 'NUMBER',   id => '3',  add => 'S',   edit => 'S',   valor => '0',   filtro => 'N',   key => 'N'),
                array( campo => 'baseimponible',        tipo => 'NUMBER',   id => '4',  add => 'S',   edit => 'S',   valor => $baseimponible,   filtro => 'N',   key => 'N'),
                array( campo => 'tarifa',               tipo => 'NUMBER',   id => '5',  add => 'S',   edit => 'S',   valor => '12',   filtro => 'N',   key => 'N'),
                array( campo => 'valor',                tipo => 'NUMBER',   id => '6',  add => 'S',   edit => 'S',   valor => $valor,   filtro => 'N',   key => 'N')
            );
    ...
    //------------------------------------------
    function _10_Cabecera_Adicional( $codFac,$correoUsuario){
    ...
    function _03_Cabecera_Factura($id){
        ...
        $filename = 'https://g-kaipi.com/kaipi/kventas/controller/datas_val.csv';
        ...
        while (($data = fgetcsv($handle, 1000, ";")) !== FALSE)
        {
            ...
            $this->ArrayAutorizacion ['cab_estado_comprobante'] = $data[0];
            $this->ArrayAutorizacion ['cab_autorizacion']= $data[1];
            $this->ArrayAutorizacion ['cab_codigo'] = $data[2];
            $this->ArrayAutorizacion ['codigo'] = $data[3];
        } 
    ...
    function _id( ){
        ...
        return $this->ArrayAutorizacion ['codigo'];
    ...
    //-----------------------------------------
    function _11_ActualizaMovimiento( $id,$tipo_dato ){
    ...
    //-----------------------------------------
    function _ActualizaAutorizacion( $id,$tipo_dato,$cab_codigo ){
    ...
    function _01_Verifica_estado( $id ){
        ...
        if ($ArrayVer['envio'] == 'S'){
            ...
            $VerAutoriza = $this->bdFactura->query_array(
                'spo_cabecera',
                "cab_estado_comprobante",
                ' emi01codi ='.$this->bd->sqlvalue_inyeccion($id,true).' and
                    RUC ='.$this->bd->sqlvalue_inyeccion($this->ruc ,true)
                );
            
            if ( $VerAutoriza['envio'] == 'AUTORIZADO'){
                return 'S';
            }else{
                return 'N';
            }
...
if ($tipo == 1) {
     ...
     $data = $gestion->_03_Cabecera_Factura();
    
    if ( $data == 'AUTORIZADO'){
      
        $id =   $gestion->_id();
        
        $gestion->_11_ActualizaMovimiento( $id ,'S');
        
        $cab_codigo = $gestion->_cab_codigo( );
        
        $gestion->_ActualizaAutorizacion( $id ,$gestion->_Autorizacion(),$cab_codigo);
    }
...
$data = 'Solicite al administrador la parametrizacion para envio de la facturacion electronica ' .$key;

if ($key == 1) {
    
    $verifica = $gestion->_01_Verifica_estado( $id );
    
    $data = 'Factura generando autorizacion....'.$verifica;
    
    
    if (trim($verifica) <> 'S'){
        
        
        $gestion->_03_Cabecera_Factura($id);  // datos de la factura
        
        $gestion->_04_TotalFactura( $id ); // sumatoria de valores
        
        $codFac = $gestion->_05_Cabecera_FacturaElectronica( $id ); // cabecera transaccion
        
        $estado = $gestion->_02_CodigoAutorizacion( $codFac,$id,0 );
        
        $estado = $gestion->_02_CodigoAutorizacion( $codFac,$id,0 );
        
        
        
        $data = '01 Factura generando autorizacion....  '.$codFac.' Estado '. $estado. ' '.$gestion->_Autorizacion();
        ...
        $estado = $gestion->_02_CodigoAutorizacion( 0,$id,1 );
        
        $codFac =  $gestion->_cab_codigo();
        
        $data = '02 Factura Ya enviada.... '.$codFac.' Estado '. $estado. ' '. $gestion->_Autorizacion();
        //-------- ---------------------------------------------
        if (trim($estado) == 'AUTORIZADO'){
            
            $gestion->_ActualizaAutorizacion( $id ,$gestion->_Autorizacion());
            
        }
        //-------------------------------------------------
        if ($estado == 'DEVUELTA'){
            
            $gestion->_11_ActualizaMovimiento( $id ,'N');
            $gestion->__eliminar($codFac);
            $data = '03 Factura en '.$estado.' vuelva a procesar '.$codFac;
