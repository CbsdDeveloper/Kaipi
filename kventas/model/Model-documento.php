...
class proceso{
	...
	function proceso( ){
		...
		$this->tabla 	  		    = 'wk_proceso_documento';
	...
	function consultaId($accion,$id ){
		...
		$datos = $this->bd->JqueryArrayVisor('wk_proceso_documento',$qquery );
	...
	function eliminar($id,$idproceso){
		...
		if ($datos['publica'] == 'N'){
			...
			$sqlDel = "DELETE FROM wk_proceso_documento  where idproceso_docu=".$this->bd->sqlvalue_inyeccion($id,true);
			
			$resultado = $this->bd->ejecutar($sqlDel);
			
			$guardarDocumento= $this->div_resultado('eliminado',$id,1);
			
		}else{
			$guardarDocumento= 'No se puede eliminar el registro';
		}
