...
class proceso{
	...
	/*	
		...
		*/
		...
		$this->tabla 	  		    = 'wk_proceso_formulario';
	...
	function xcrud($idtarea,$idproceso){
		...
		$flujo = $this->bd->query_array('wk_proceso_formulario',
				...
				'idproceso='.$this->bd->sqlvalue_inyeccion($idproceso,true).' and idtarea  = '.$this->bd->sqlvalue_inyeccion($idtarea,true)
				);
	...
	function agregar( $idtarea,$idproceso){
		...
		$sqlDepa = "SELECT idproceso, idproceso_var, variable
									  ...
									  where estado = 'S' AND idproceso = ".$this->bd->sqlvalue_inyeccion($idproceso,true). " order by orden";
		...
		$this->bd->JqueryUpdateSQL('wk_procesoflujo',$UpdateQuery);
	...
	function edicion($idtarea,$idproceso){
		...
		while ($x=$this->bd->obtener_fila($stmt_depa)){
            ...
            //-------------
 			$Avalida = $this->bd->query_array('wk_proceso_formulario',
			                                  'count(*) as nn', 
