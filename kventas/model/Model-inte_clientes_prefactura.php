...
class proceso{
    ...
    function consultaId( $id,$idvengestion ){
        ...
        if ( $datos['actualizado'] == 'S'){
            ...
            $Aestado = $this->bd->query_array('ven_cliente_seg',
                                                 'estado,novedad,vendedor', 
                                                 'idvengestion='.$this->bd->sqlvalue_inyeccion($idvengestion,true)
                                                );
            
            $comision = $Aestado['vendedor'];
            
            if ($Aestado['estado']  == 11){
                
                if ( $datos['idmovimiento'] == 0  ){
                         // movimiento
                       $id_movimiento =  $this->agregar($datos,$identificacion,$idvengestion,$Aestado['novedad'],$comision);
                        ...
                        if ( $id_movimiento == 0){
                            ...
                            $sqlEdit = "update ven_cliente
                               set proceso  =".$this->bd->sqlvalue_inyeccion( 'Facturacion emitida',true).",
                    ...
                    $dato = '<b>Ya se emitio el comprobante de '.$datos['idmovimiento'] .'!!! </b>';
                }
    ...
    function agregar($datos,$identificacion,$idvengestion,$novedad,$comision){
        ...
        $AResultado = $this->bd->query_array('ven_cliente_prod',
                                             ...
                                             'estado = 0 and idvengestion='.$this->bd->sqlvalue_inyeccion($idvengestion,true).' and 
                                             registro='.$this->bd->sqlvalue_inyeccion( $this->ruc ,true)
            );
