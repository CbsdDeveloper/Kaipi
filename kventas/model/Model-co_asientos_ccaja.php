...
class proceso{
    ...
    function agregar( $id_asiento_old ){
        ...
        $sql = "INSERT INTO co_asiento(	fecha, registro, anio, mes, id_tramite,detalle, sesion, creacion,
                						comprobante, estado, tipo, documento,modulo,marca,id_periodo)
										        VALUES (". $this->hoy.",".
										        ...
										        $this->bd->sqlvalue_inyeccion('Deposito /Cierre de caja '.$x["detalle"], true).",".
										        ...
										        $idAsiento1 =  $this->bd->ultima_secuencia('co_asiento');
										        
										        $result = ' <b>Asiento generado Nro.'.$idAsiento1.' </b>';
										        
										        if  ( !empty(trim($idAsiento1))) {
										            
										            $this->agregarDetalle( $idAsiento1,$id_asiento_old);
										            
										            $sql1 = "update co_asiento set marca='C' 
                                                             where id_asiento=".$this->bd->sqlvalue_inyeccion($id_asiento_old, true);
