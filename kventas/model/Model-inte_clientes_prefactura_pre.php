...
class proceso{
    ...
    //--------------------------------------------------------------------------------
    function consultaId( $idprov ){
          
        
        $sql ="SELECT  max(a.idprov) as idprov,max(a.razon) as razon, max(a.idven_gestion) as idven_gestion
                 FROM  ven_cliente_gestion a 
                where a.idmovimiento = -1 and a.factura = 'S' and
                      ...
                      a.bandera =" .$this->bd->sqlvalue_inyeccion('S',true);
        ...
        while ($fetch=$this->bd->obtener_fila($resultado)){
            ...
            $idven_gestion =  $fetch['idven_gestion'];
             $qquery = array(
                array( campo => 'idven_gestion',   valor => $idven_gestion,  filtro => 'S',   visor => 'N'),
        ...
        $id_movimiento =  $this->agregar($datos,$idprov,$idven_gestion,'Facturacion',$comision);
        
        //------------ detalle adicionales------------
        
        $sql1 ="SELECT a.idmovimiento,a.fecha,a.idprov,a.razon, a.idven_gestion, a.bandera
                 FROM  ven_cliente_gestion a
                where a.idmovimiento = -1 and a.factura = 'S' and
                      a.registro =" .$this->bd->sqlvalue_inyeccion($this->ruc,true)." and
                      a.idprov =" .$this->bd->sqlvalue_inyeccion(trim($idprov),true)." and
                      a.bandera =" .$this->bd->sqlvalue_inyeccion('S',true) ;
        
         
        $resultadoDet = $this->bd->ejecutar($sql1);
        
        while ($x=$this->bd->obtener_fila($resultadoDet)){
            
            
            $idven_gestion =  $x['idven_gestion'];
            
            $sqlProd = "SELECT  idproducto, idbodega,  producto, detalle,   cantidad,precio_descuento,iva,subtotal,total, tipo
                                FROM  ven_cliente_prod
                                where estado = 1 and
                                      idvengestion = ".$this->bd->sqlvalue_inyeccion($idven_gestion,true)." and
                                      registro = ".$this->bd->sqlvalue_inyeccion($this->ruc,true) ;
            
   
            $stmt_prod = $this->bd->ejecutar($sqlProd);
            // inserta detalle
            while ($fila=$this->bd->obtener_fila($stmt_prod)){
            ...
            $sqlEdit = "update ven_cliente_gestion
                               set factura  =".$this->bd->sqlvalue_inyeccion( 'S',true).",
                                   bandera  =".$this->bd->sqlvalue_inyeccion( 'N',true).",
                                   idmovimiento  =".$this->bd->sqlvalue_inyeccion( $id_movimiento,true)."
                             where idven_gestion = ".$this->bd->sqlvalue_inyeccion($idven_gestion,true)  ;
        ...
        $dato = 'OK..Dato Procesado genere la facturacion';
        return  $dato;
...
if (isset($_GET['idprov']))	{
    ...
    $ViewPendiente =   $gestion->consultaId(trim($id));
