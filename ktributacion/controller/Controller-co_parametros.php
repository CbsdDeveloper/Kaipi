    ...
    class componente{
     ...
     function Formulario( ){
        ...
        $this->set->body_tab($titulo,'inicio');
                ...
                $this->set->div_panel('<b> PARAMETROS TRIBUTARIOS</b>');
                ...
                $resultado =  $this->bd->ejecutar("select '-' as codigo, 'No Aplica' as nombre
        								 from co_plan_ctas
        								 where univel = 'N' and cuenta = ". $this->bd->sqlvalue_inyeccion('1' ,true)."
        								 union
        							     select cuenta as codigo, trim(detalle) as nombre
        								 from co_plan_ctas
        								 where univel = 'S' and registro =". $this->bd->sqlvalue_inyeccion($id ,true)." order by 1");
                
                
                $tipo =  $this->bd->retorna_tipo();
                
                $this->obj->list->listadb($resultado,$tipo,'Cuenta Contable','cuenta',$datos,'required','','div-2-10');	 
