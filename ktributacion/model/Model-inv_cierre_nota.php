...
class proceso{
	...
	function consultaId($accion,$id ){
		...
		$datos = $this->bd->JqueryArrayVisorDato('view_ventas_fac',$qquery );
		

		
		$ADatos = $this->bd->query_array(
		    'web_registro',
		    'razon, contacto, correo,direccion,felectronica,estab,puntoe',
		...
		$numdocmodificado = $ADatos['estab'].'-'.$ADatos['puntoe'].'-'.$datos['comprobante'];
		
		$xn = $this->bd->query_array(
		    'doctor_vta',
		    'secuencial1,cab_autorizacion,id_diario,fechaemisiondocsustento,nombre,comprobante1',
		    'id_diario='.$this->bd->sqlvalue_inyeccion( $id,true)
		    );
		...
		if ( trim($nombre) == 'NC-AN'){
			$datos['comprobante'] = $xn['comprobante1'];
		} 
		
		if ( empty($xn['fechaemisiondocsustento'])){
		    
		    $fechaemisiondocsustento = $this->hoy 	;
		...
		echo "<script>
            	$('#estab1').val("."'".$ADatos['estab']."'".");
                $('#ptoemi1').val("."'".$ADatos['puntoe']."'".");
                $('#fechaemisiondocsustento').val("."'".$fechaemisiondocsustento."'".");
                $('#coddocmodificado').val("."'".'01'."'".");
                $('#numdocmodificado').val("."'".$numdocmodificado."'".");
                $('#secuencial1').val("."'".$secuencial1."'".");
                $('#cab_autorizacion').val("."'".$cab_autorizacion."'".");
                $('#id_movimiento1').val("."'".$id."'".");
                $('#id_diario').val("."'".$id_diario."'".");
                $('#idcliente').val("."'".$datos['idprov']."'".");
                $('#comprobante1').val("."'".$datos['comprobante']."'".");
                $('#fecha_factura').val("."'".$datos['fecha']."'".");
         	 </script>";
		
	 
	 
  
		$resultadoNota = $datos['fecha'];
	...
	//--------------------------------------------------------------------------------
			...
			function AnulaNota($id,$secuencial1,$comprobante1 ){
 
				echo $id.'  '.$secuencial1 ;

				$comprobante_anula = substr(trim($comprobante1 ),2,9);
				$comprobante_anula = 'A'.$comprobante_anula ;
 
			 	$result = '<img src="../../kimages/kdel.png"/>&nbsp;<b>NOTA DE CREDITA ANULADA NRO  ['.$id.']</b><br>';
				...
				$sql = " UPDATE doctor_vta
								   SET 	comprobante1 =".$this->bd->sqlvalue_inyeccion($comprobante_anula , true)." ,
								   		numdocmodificado =".$this->bd->sqlvalue_inyeccion('Anulado-'.$comprobante1 , true)." ,
										cuenta =".$this->bd->sqlvalue_inyeccion('A' , true)." ,
								        nombre =".$this->bd->sqlvalue_inyeccion('NC-AN', true)." ,
										id_diario = (id_diario *-1)
 								   WHERE id_diario=".$this->bd->sqlvalue_inyeccion($id, true). " and 
...
*/			
	 ...
	 */
	        $resultado = '<img src="../../kimages/kfav.png" align="absmiddle" />&nbsp;<b>CAJA CERRADA ['.$this->sesion.']</b><br>';
	        
 	        
	    }else{
	        
	        $resultado = '<img src="../../kimages/ksavee.png" align="absmiddle" />&nbsp;<b>REGISTRE NOVEDAD EN EL CIERRE ['.$this->sesion.']</b><br>';
...
if (isset($_GET['accion']))	{
	...
	if ($accion == 'nota'){
		...
		$comprobante1 = $_GET['comprobante1'];
		if ($accion == 'anula'){
			$gestion->AnulaNota($id,$secuencial1 ,$comprobante1);
		}
		else {
