	...
	if ($accion == 'add'){
	    ...
	    if ( $id_diario > 0 )  {
	     ...
	     $secuencial1 =  nuevo(  $bd, $registro,$id,$idcliente,$comprobante1,$fecha_factura,
	                            ...
	                            $coddocmodificado,$numdocmodificado,$idmov);
...
//--- funciones grud	
	...
	function nuevo($bd, $registro,$id,$idcliente,$comprobante1,$fecha_factura,$secuencial1,
	    ...
	    $x = $bd->query_array('doctor_vta',
	                          'max(cast(secuencial1 as int)) as nn', 
	    ...
	    $ATabla = array(
	        ...
	        array( campo => 'secuencial',tipo => 'VARCHAR2',id => '7',add => 'S', edit => 'N', valor => $secuencial1, key => 'N'),
	        ...
	        array( campo => 'secuencial1',tipo => 'VARCHAR2',id => '17',add => 'S', edit => 'N', valor => $secuencial1, key => 'N'),
	 ...
	 function guarda_detalle_nota($id, $bd){
...
where   id_movimiento = '.$bd->sqlvalue_inyeccion($id, true). ' order by id desc';
...
$stmt = $bd->ejecutar($sql);
	...
	while ($x=$bd->obtener_fila($stmt)){
				...
				$InsertQuery = array(   
					array( campo => 'id_movimiento',   valor => $id),
					array( campo => 'idproducto',   valor =>$x["idproducto"] ),
					array( campo => 'cantidad',   valor => $x["cantidad"] ),
					array( campo => 'costo',   valor => $x["costo"]),
					array( campo => 'total',   valor =>  $x["total"]),
					array( campo => 'baseiva',     valor =>   $x["baseiva"]),
					array( campo => 'monto_iva',     valor =>  $x["monto_iva"]),
					array( campo => 'tarifa_cero',   valor =>  $x["tarifa_cero"]) 
				);

 

		$bd->JqueryInsertSQL('inv_movimiento_det_nota',$InsertQuery);
