...
class proceso{
    ...
    <tbody>
	  ...
	  <tr>
      <td colspan="2">Certifico que la información contenida en el medio magnético del Anexo   Transaccional para el período '.$mes.'-'.$anio.', es fiel reflejo del siguiente reporte:</td>
      </tr>
      <tr>
      <td>&nbsp;</td>
      <td>&nbsp;</td>
    ...
    function Valor($variable,$tipo){
        ...
        if ($tipo == 'N'){
            ...
            $variable = $this->bd->numero($valor,2);
            
            return $this->bd->sqlvalue_inyeccion($variable, true);
        }
        
        
        else{
            
            return $this->bd->sqlvalue_inyeccion(@$_POST[$variable], true);
